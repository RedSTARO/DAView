...
**播放**
- Android：Media3 / ExoPlayer 内置播放器。手势：单击呼出控件、左右两侧双击 ±10 秒、中间双击暂停、
  横向拖动快进快退、左半边上下拖亮度、右半边上下拖音量、长按 2 倍速；另有缓冲指示、上一集 / 下一集、
  锁屏防误触、锁定横屏、画中画、倍速
...
- 桌面 + NVIDIA：**RTX Video Super Resolution** 与 **RTX Video HDR**
  （见「桌面端的应用内播放」）
- 音轨与字幕：可以设首选语言；同一部剧里上一集选过的轨道（包括「关闭字幕」）下一集沿用
- 播放中可以从本机加载一个字幕文件（ASS / SSA / SRT / VTT，只对这一次播放有效）
- MKV 的章节在第一次播放时读出并存下；播放器里有章节列表，发布组把片头标成章节
  （`OP`、`Opening`、`片头`…）时出现「跳过片头」
- 断点续播、已看标记（超过 90% 自动标记已看）、收藏、继续观看 / 接下来 / 最近添加
- 一集播完后倒数 8 秒接着播下一集，可以取消；连续自动播了 3 集没人碰，会停下来问一次
...
**离线下载**
- 单集、整季、整部剧都能下载到本机：详情页的下载按钮、卡片的右键 / 长按菜单、媒体库页的多选。
  整季 / 整部会先算一遍还差多少集、多少 GB 再问一次，已经在本机的分集不重复下
- **外挂字幕跟着视频一起下**。字幕在网盘上是独立的文件，只下视频等于离线时没有字幕；
  所以一次下载 = 视频 + 扫描时找到的每一个外挂字幕（包括 `Subs/` 目录里的），字幕落在视频旁边、
  沿用网盘上的文件名，外置播放器打开文件夹也能自动挂上。海报与背景图同时进图片缓存，
  详情页离线也有图
- 某个字幕下不下来（比如网盘上已经没了）不会拖累视频：视频照常完成，列表里标「1 个字幕未能下载」，
  下次再点下载只补字幕
- 播放、探测容器、读章节与 Cues 都先看本机有没有：下载完成后播这一集不发任何网络请求，
  外置播放器的进度推算照常工作
- **断点续传**：传输写到 `.part` 文件、最后一字节才改回真名，进程被杀、断网、切到移动数据都只损失
  当前块；应用下次启动会把上次没做完的队列接着做（旧版本只下了视频的记录也会自动补字幕）
- 文件按网盘目录结构落盘：`<下载目录>/Ani/Bocchi the Rock! (2022)/Season 01/…`，文件名对 Windows
  不合法的字符会被替换
- 桌面：可以改下载目录（之后的下载放新目录，已有的留在原处照常播）、在资源管理器 / Finder 里定位文件
- Android：前台服务加通知栏进度（通知上可以「全部取消」）；默认**仅在 Wi-Fi 下下载**，用移动数据时
  排队等着、连上 Wi-Fi 从中断处继续；文件放在应用自己的存储空间，卸载即清除
- 一次只传一个：两个下载共用同一条带宽，总时间一样，但顺序传第一集能更早看
- 下载只属于这台设备，不进同步文件
...
### 控件画在哪：盖在画面上的一个独立透明窗口

「盖不到视频上」只对**同一个窗口**成立。所以控件条放进另一个窗口：无边框、背景透明、
永远不拿焦点的 `ComposeDialog`，归主窗口所有，贴在视频区域的底部，跟着主窗口移动、缩放、
进出全屏。两个顶层窗口由系统合成器叠在一起，控件出现和收起时**画面尺寸不变**。
早先控件条占着布局，它一出一进，画面就缩放一次；音轨和字幕也只能一条条循环。

| 东西 | 画在哪 | 怎么触发 |
| --- | --- | --- |
| 上一集 / 播放暂停 / 下一集、进度条、音量、倍速、章节、音轨、字幕、字幕大小与延迟、加载字幕文件、全屏、结束播放 | 浮在画面底部的控件窗口 | 鼠标一动就出来，停 3 秒收起；暂停、打开了列表、鼠标停在控件上时不收 |
| 调音量 / 倍速 / 字幕延迟时的即时读数，控件收起时的缓冲进度 | mpv 的 OSD（`show-text`） | 自动 |
| `#` / `j` 循环切轨时的整张轨道列表 | mpv 的 OSD | `#` / `j` |
| RTX 超分 / HDR 结果 | 起播 2.5 秒后在 OSD 报一次 | 自动 |
| 跳过片头 | 画面右下角另一个小浮窗 | 当前章节是片头、且控件收起时出现；Enter 也行 |

进度条上标出章节位置，悬停显示那一点的时间，点右侧时间在「总长 / 剩余」之间切换。
音轨和字幕是能点选的列表，当前项带勾；`#` / `j` 仍然是「切到下一条」，给习惯 mpv 的人。

平台做不到逐像素透明时，控件条落在自己那条深色底上——仍然盖在画面上，不挤画面。

**mpv 自带的 OSC 在这里用不了，所以根本不开。** 实测：嵌在别人窗口里的 mpv
收不到任何鼠标事件——整部片里 `mouse-pos` 一直是 `{x:0,y:0,hover:false}`，连
`input-cursor=yes` 也改不了。于是 OSC 的默认 `auto` 永远等不到那一下鼠标移动；
强行 `osc-visibility always` 把它叫出来，按钮也一样不响应点击。所以传输控件只能
DAView 自己画。

鼠标因此只有一个地方能抓到：重量级 AWT `Canvas` 上的监听器。控件条靠它唤回来；
画面上单击暂停（等过双击间隔才生效，否则双击会先暂停再恢复）、双击切全屏、滚轮调音量。
全屏且控件收起时，鼠标指针也一起隐藏。

> 坐标要取 `xOnScreen` 而不是组件坐标，否则有一个自己转起来的环：画面一变，
> AWT 把「组件在鼠标下面动了」报成鼠标移动，控件条又回来了。

**键两边都接。** 键盘焦点可能在 DAView 的窗口，也可能在 mpv 的子窗口，所以同一组键两边都绑：
Compose 这边直接处理；mpv 那边要么用 `keybind` 指回 DAView（`script-message` → 客户端消息），
要么直接绑成 mpv 命令（方向键），其余的是 mpv 自己的默认键。
...
| `<` / `>` | 上一集 / 下一集 |
| Enter | 跳过片头 |
| Esc | 先关掉打开的列表，再退出全屏；**不会**结束播放 |
| Q | 结束播放 |

反过来也读：每 5 秒把 mpv 的 `aid` / `sid` 读回来映射成 DAView 的索引。mpv 自己也会
选轨道，默认键也还在，不读回来的话在 mpv 里换的轨道根本不会被记录，
换台设备续播时就落在没人选过的那条上。
...
- **离线下载只在单元测试里跑过**：`OfflineLibraryTest` 用内存里的字节源验过整条链路，真实网盘、Android
  的前台服务与 Wi-Fi 门控、桌面的目录选择与「在文件夹中显示」都只编译过，没在真机上跑。
  Android 上「等 Wi-Fi」靠 `ConnectivityManager` 的 `NOT_METERED` 能力判断，热点被系统标成计费时也会等。
- **改下载目录不搬已有文件**：每条记录记着自己的绝对路径，旧文件留在旧目录照常播放；想搬得删了重下。
...
- **桌面的控件浮窗只用测试信号源跑过**：在一台 Windows（RX 7900 XT）上用 mpv 的 `lavfi` 测试源确认了
  窗口和全屏下控件都浮在画面上、控件出没时画面尺寸不变、全屏进出正常；真实片源、HDR 与 RTX 路径下没跑过。
  平台不支持逐像素透明时，控件条会是一条深色底，但仍盖在画面上。
- **控件浮窗要避开 skiko 的一个行为**：skiko 给每个 Compose 窗口挂的 `FullscreenAdapter` 会在窗口显示时
  调 `setFullScreenWindow(null)`，把整块屏幕上的全屏窗口踢出全屏。浮窗创建时就把它摘掉
  （`keepOwnerFullscreen`）；以后再加别的 Compose 窗口也要这么做，否则一弹出来主窗口就退出全屏。
...
特典季的排序（`Season 00` 不得抢在第 1 季前面，但「没有季」不等于第 0 季），
轨道沿用（上一集选的轨道按语言、外挂 / 内封、标题对到这一集；「关闭字幕」也沿用），
手动编辑过的字段不被重新刮削覆盖，MKV 章节的读取（默认版本、跳过隐藏章节、按时间排序），
以及离线下载（字幕跟着视频落到同一目录、某个字幕下不下来不拖累视频、从 `.part` 的长度续传而不是重来、
等 Wi-Fi 时取消要连文件夹一起清掉、上次进程留下的队列接着做、只下了视频的旧记录补字幕不重下视频、
文件名对 Windows 不合法的字符被替换）。
