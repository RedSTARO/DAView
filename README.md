...
| 东西 | 画在哪 | 怎么触发 |
| --- | --- | --- |
| 进度条 / 播放暂停 / 音量 | mpv 自带的 OSC | mpv 自己 |
| 正在播放的片名 | mpv 的 OSC（`force-media-title`） | 起播时写入，否则那里显示的是 `127.0.0.1` 的地址 |
| 音轨 / 字幕列表 | mpv 的 OSD（`show-text`） | `#` / `j`，或非全屏时工具条上的按钮 |
| RTX 超分 / HDR 结果 | 起播 2.5 秒后在 OSD 报一次 | 自动 |

OSD 消息不占布局，所以**画面再也不会被推下去**；全屏时 Compose 那条工具条直接不渲染，
窗口里只有画面。工具条也不做「几秒后淡出」——它一出现一消失就会让视频改变尺寸，
没有哪个播放器会这样对待观众。
...
- **桌面切轨是「切到下一条」，不是「从列表里挑」**：列表画在 mpv 的 OSD 上，而 OSD 只是一张图，
  收不到鼠标点击。所以选轨道靠重复按 `#` / `j`（或非全屏时重复点工具条上的按钮），
  轨道多的时候要按好几下。全屏时那条工具条不渲染，鼠标没有入口，只能用键盘。
  这是「画面不被挤下去」换来的代价：能被点的控件必须占布局，占布局就会挤画面。
