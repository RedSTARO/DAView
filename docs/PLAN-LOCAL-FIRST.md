...
| `39b66f6` | 字幕地址跟着请求它的会话释放（原先挂在一个常量会话号上）|
| `3cf41f7` | 会话以任何方式结束都关掉管道；补上两处只审过没测过的判断 |
...
- `./gradlew :core:jvmTest`：47 个测试全绿（其中 19 个是这轮新增的）
...
管道的存活期是最后补的一处。它原先只在 `stopPlayback` 时关闭，而外置播放器在
Android 上根本观察不到进程退出，会话是被 `PlaybackService` 的空闲超时收走的，
那条路径谁也没通知——端口会一直留到进程结束，正好是这次改造要去掉的东西。现在
会话结束由 `PlaybackService` 广播，管道自己订阅，不再依赖界面做对。
